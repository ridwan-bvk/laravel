    ...
    @if ($posts->count())
        <div class="card mb-3">
            ...
            <div class="card-body text-center">
                <h3 class="card-title"><a href="/post/{{ $posts[0]->id }}"
                        class="text-decoration-none text-dark">{{ $posts[0]->title }}</a></h3>
                <small class="text-muted">
                    <p> By: <a href="/authors/{{ $posts[0]->user->username }}"
                            class="text-decoration-none">{{ $posts[0]->user->name }}</a> in
                        {{-- <a href="/categories/{{ $posts[0]->category->id }}" class="text-decoration-none"> --}}
                    ...
                    <div class="card-body">
                        ...
                        <small class="text-muted">
                            <p> By: <a href="/authors/{{ $post->user->username }}"
                                    ...
                                    {{-- rubah dengan category search --}}
                                {{-- <a href="/categories/{{ $post->category->id }}" class="text-decoration-none"> --}}
...
<div class="d-flex justify-content-end">
    ...
    {{-- @foreach ($posts->skip(1) as $post)
        <article class="mb-5 border-bottom pb-4">
            ...
            {{-- <p>{{ $pos->body }}</p> --}}
    {{-- <h6> By: <a href="/authors/{{ $post->user->username }}"
                    class="text-decoration-none">{{ $post->user->name }}</a> in
                <a href="/categories/{{ $post->category->id }}" class="text-decoration-none">
                    {{ $post->category->name }} </a>
            </h6>
            <p>{{ $post->excerpt }}</p>
            <a href="/posts/{{ $post->id }}" class="text-decoration-none">read more</a>
